...
double precision,allocatable,dimension(:,:) :: &
 ...
 if( myrank == 0 ) write(*,*)" WS-lab nnp-training program, version 20201213.1"
...
1112 format(I10,I3,A3,A4,I2,A4,I2,A4,I2,I4)
 ...
 elseif( io_list == 1 )then
   ...
   if( myrank == 0 )then
     ...
     do i = 1, num_data_total_mpi
       read(98,*) dummyi, list_mpi(i), dummyc, &
                  dummyc, info_io_mpi((i-1)*3+1), &
                  dummyc, info_io_mpi((i-1)*3+2), &
                  dummyc, info_io_mpi((i-1)*3+3)
     enddo
     close(98)
   endif ! myrank == 0
   ...
   call mpi_scatterv( info_io_mpi, recvcount, displs, mpi_integer, &
                      info_io, num_data_total*3, mpi_integer, &
                      0, mpi_comm_world, ierr )
   call mpi_barrier( mpi_comm_world, ierr )
